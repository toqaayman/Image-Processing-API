...
describe('Test endpoint response', () => {
  ...
  it('Request is rejected and display Expected to receive a number for height but instead received a character', async () => {
    const response = await request.get('/api/image?filename=fjord&height=x&width=500')
    expect(response.status).toBe(400)
  })
})
describe('Test image resize function', () => {
  ...
  it('It should reject any height or width that is not positive or equal to zero', () => {
    expect(ImageChecker.PositiveNeg(minusHeight, width)).toBeFalse()
  })
